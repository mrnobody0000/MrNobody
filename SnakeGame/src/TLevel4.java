...
class TLevel4 extends JPanel implements ActionListener{
    ...
    TLevel4(int sond,int lvl,int sp,int ap) throws IOException, UnsupportedAudioFileException, LineUnavailableException {
    ...
    public void draw(Graphics g) throws IOException, InterruptedException {
        ...
        if(running) {
            ...
            g.drawString(" TOURNAMENT : RAILS ", 20, 645);
    ...
    public void checkApple() throws UnsupportedAudioFileException, LineUnavailableException, IOException {
        if((x[0] == appleX) && (y[0] == appleY)) {
            ...
            if(applesEaten>8)
            {
                clip.stop();
                clip.close();
                new TGameFrame5(runningSound,levelSelector,DELAY,applesEaten);
