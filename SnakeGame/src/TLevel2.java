...
class TLevel2 extends JPanel implements ActionListener{
    ...
    TLevel2(int sond,int lvl,int sp,int ap) throws IOException, UnsupportedAudioFileException, LineUnavailableException {
    ...
    public void draw(Graphics g) throws IOException, InterruptedException {
        ...
        if(running) {
            ...
            g.drawImage(brick.getImage(), 600,600,  UNIT_SIZE, UNIT_SIZE,null);


            for(int i=120;i<495;i+=15)
            {
            ...
            g.drawString(" TOURNAMENT : TUNNEL ", 20, 645);
    ...
    public void newApple() {
        ...
        if(appleY< 15&&(appleX<180|| appleX>405))
            newApple();


        if(appleY > SCREEN_HEIGHT-15&&(appleX<165|| appleX>420))
            newApple();
    ...
    public void checkApple() throws UnsupportedAudioFileException, LineUnavailableException, IOException {
        if((x[0] == appleX) && (y[0] == appleY)) {
            ...
            if(applesEaten>4)
            {
                clip.stop();
                clip.close();
                new TGameFrame3(runningSound,levelSelector,DELAY,applesEaten);
    ...
    public void checkCollisions() {
        ...
        if(y[0] < 15&&(x[0]<180|| x[0]>405))
            running = false;
        if(y[0]<0&& (x[0]>180|| x[0]<480))
            y[0]=SCREEN_HEIGHT;

        if(y[0] > SCREEN_HEIGHT-15&&(x[0]<165|| x[0]>420))
            running = false;
        if(y[0]>SCREEN_WIDTH &&( x[0]>180|| x[0]<420))
            y[0]=0;
